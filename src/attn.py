...
class GQAAttention(nn.Module):
    ...
    def _sliding_window_mask(self, T: int, device):
        ...
        mask = i.unsqueeze(1) - j.unsqueeze(0)
        causal = mask < 0
        sw = (
    ...
    def forward(
        ...
        # -------- RoPE: apply full range to k, tail slice to q --------
        ...
        assert y.size(1) == T_q, "unexpected time dim after attention"
        y = y.view(B, T_q, H * D)
        y = self.wo(y)
